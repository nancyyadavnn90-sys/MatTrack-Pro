    ...
    <div className="bg-[#121212] text-slate-200 min-h-screen p-6 font-sans space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#2a2a2a] pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-black tracking-wide flex items-center gap-2 text-white">
              Jayashree <span className="text-emerald-400 font-extrabold text-xs bg-[#1e1e1e] border border-[#2a2a2a] px-2 py-0.5 rounded-md">WIP KANBAN BOARD</span>
            </h1>
      ...
      {activeTab === 'kanban' && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 w-full">
          ...
          {/* LEFT SIDE: KANBAN COLUMNS AND FILTERS (9 Columns wide) */}
          <div className="xl:col-span-9 space-y-4 w-full">
            
            {/* Filters Row */}
            <div className="bg-[#1e1e1e] p-4 border border-[#2a2a2a] rounded-xl flex flex-wrap items-center justify-between gap-4 shadow-md">
              <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-slate-300">
                <div>
                  <label className="text-[10px] text-slate-400 block mb-1">Customer Filter</label>
                  <select
                    value={filterCustomer}
                    onChange={(e) => setFilterCustomer(e.target.value)}
                    className="px-3 py-1.5 bg-[#121212] border border-[#3a3a3a] rounded-lg focus:outline-none text-xs text-white font-medium"
                  >
                ...
                <div>
                  <label className="text-[10px] text-slate-400 block mb-1">Work Order</label>
                  <select
                    value={filterWO}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-3 py-1.5 bg-[#121212] border border-[#3a3a3a] rounded-lg text-xs w-64 text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500 font-medium"
                />
              </div>
            </div>

            {/* Kanban Columns Grid */}
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin select-none h-[580px] w-full">
              {boardData.map(stage => {
                ...
                return (
                  <div 
                    key={stage.stage_id} 
                    className="flex-shrink-0 w-[290px] bg-[#1e1e1e] rounded-xl p-3.5 flex flex-col h-[550px] border border-[#2a2a2a] shadow-lg"
                  >
          ...
          {/* RIGHT SIDEBAR PANEL: MACHINE STATUS, QUICK SCAN & ACTIVITIES (3 Columns wide) */}
      ...
      {activeTab === 'reports' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#1e1e1e] p-5 rounded-xl border border-[#2a2a2a] space-y-4 md:col-span-2 shadow-lg">
            <h3 className="font-extrabold text-white text-sm border-b border-[#2a2a2a] pb-2">Work Order wise WIP Status</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {woStatusReport.map((wo, idx) => {
                ...
                return (
                  <div key={idx} className="border border-[#2a2a2a] p-4 rounded-xl space-y-2.5 text-xs font-semibold text-slate-300 bg-[#121212]">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="font-extrabold text-white block text-sm">{wo.wo_number}</span>
                        <span className="text-[10px] text-slate-400 block font-normal">{wo.item_name} • {wo.customer_name}</span>
                      </div>
                      <span className="bg-emerald-500/10 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded font-black text-[10px]">
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between items-center text-[10px]">
                        ...
                        <strong className="text-white">{progress}% ({wo.produced_qty}/{wo.planned_qty} Pcs)</strong>
                      </div>
                      <div className="w-full bg-[#1e1e1e] h-2 rounded-full overflow-hidden border border-[#2a2a2a]">
                        <div className="bg-[#10b981] h-full rounded-full" style={{ width: `${progress}%` }}></div>
                      </div>
                    </div>
                  </div>
                );
      ...
      <div className="space-y-6">
        ...
        {/* TOP ROW: WIP SUMMARY (6 cols) + PRODUCTION PERFORMANCE (6 cols) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          
          {/* Doughnut Chart Panel (6 Columns) */}
          <div className="lg:col-span-6 bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-5 space-y-4 shadow-lg">
            <div className="border-b border-[#2a2a2a] pb-2">
              <span className="text-xs text-slate-400 uppercase tracking-wider font-black block">WIP Summary</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 justify-center py-2">
              ...
              <div className="relative w-32 h-32 flex items-center justify-center flex-shrink-0">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                  <circle cx="18" cy="18" r="15.915" fill="none" stroke="#121212" strokeWidth="3" />
                  <circle cx="18" cy="18" r="15.915" fill="none" stroke="#3b82f6" strokeWidth="3.2" strokeDasharray="18 82" strokeDashoffset="100" />
                  <circle cx="18" cy="18" r="15.915" fill="none" stroke="#10b981" strokeWidth="3.2" strokeDasharray="27 73" strokeDashoffset="82" />
                  <circle cx="18" cy="18" r="15.915" fill="none" stroke="#f59e0b" strokeWidth="3.2" strokeDasharray="16 84" strokeDashoffset="55" />
                  <circle cx="18" cy="18" r="15.915" fill="none" stroke="#a855f7" strokeWidth="3.2" strokeDasharray="13 87" strokeDashoffset="39" />
                  <circle cx="18" cy="18" r="15.915" fill="none" stroke="#06b6d4" strokeWidth="3.2" strokeDasharray="11 89" strokeDashoffset="26" />
                  <circle cx="18" cy="18" r="15.915" fill="none" stroke="#ef4444" strokeWidth="3.2" strokeDasharray="7 93" strokeDashoffset="15" />
                </svg>
              ...
              {/* Legend grid */}
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs font-extrabold">
          ...
          {/* Production Performance Panel (6 Columns) */}
          <div className="lg:col-span-6 bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-5 space-y-4 shadow-lg">
            <div className="border-b border-[#2a2a2a] pb-2">
              <span className="text-xs text-slate-400 uppercase tracking-wider font-black block">Production Performance (Today)</span>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-[#121212] p-3.5 border border-[#2a2a2a] rounded-xl">
                <span className="text-slate-400 text-[10px] font-black uppercase tracking-wider block">Planned Qty</span>
                <span className="text-sm font-black text-white mt-1 block">18,000 Pcs</span>
              </div>
              <div className="bg-[#121212] p-3.5 border border-[#2a2a2a] rounded-xl">
                <span className="text-slate-400 text-[10px] font-black uppercase tracking-wider block">Produced Qty</span>
                <span className="text-sm font-black text-white mt-1 block">14,320 Pcs</span>
              </div>
              <div className="bg-[#121212] p-3.5 border border-[#2a2a2a] rounded-xl">
                <span className="text-slate-400 text-[10px] font-black uppercase tracking-wider block">Good Qty</span>
                <span className="text-sm font-black text-emerald-400 mt-1 block">13,842 Pcs</span>
              </div>
              <div className="bg-[#121212] p-3.5 border border-[#2a2a2a] rounded-xl">
                <span className="text-slate-400 text-[10px] font-black uppercase tracking-wider block">Rejected Qty</span>
                <span className="text-sm font-black text-red-400 mt-1 block">478 Pcs</span>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3">
              {/* Sparkline stats 1 */}
              <div className="bg-[#121212] p-3 border border-[#2a2a2a] rounded-xl flex items-center justify-between">
                <div>
                  <span className="text-slate-400 text-[10px] font-black uppercase tracking-wider block">Rejection %</span>
                  <span className="text-sm font-black text-red-400 mt-0.5 block">3.34%</span>
                </div>
              ...
              <div className="bg-[#121212] p-3 border border-[#2a2a2a] rounded-xl flex items-center justify-between">
                <div>
                  <span className="text-slate-400 text-[10px] font-black uppercase tracking-wider block">OEE %</span>
                  <span className="text-sm font-black text-emerald-400 mt-0.5 block">72.45%</span>
                </div>
                <svg className="w-16 h-6 text-slate-700" viewBox="0 0 100 20">
        ...
        {/* BOTTOM ROW: TOP WORK ORDERS IN PROGRESS (Full 12 Columns Wide!) */}
        <div className="lg:col-span-12 bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-5 space-y-4 shadow-lg w-full">
          <div className="border-b border-[#2a2a2a] pb-3 flex justify-between items-center">
            <span className="text-xs text-white uppercase tracking-wider font-black block">Top Work Orders In Progress</span>
            <span className="text-xs text-emerald-400 font-black uppercase">Live Status</span>
          </div>
            ...
            <table className="w-full text-left text-xs font-semibold border-collapse text-slate-200">
              <thead>
                <tr className="border-b border-[#333] text-slate-200 text-xs font-black uppercase bg-[#252525]">
                  ...
                  <th className="py-3 px-4 text-right text-emerald-400">Done Qty</th>
                  <th className="py-3 px-4 text-center">Progress</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#2a2a2a]">
                {woStatusReport.slice(0, 6).map((wo, idx) => {
                  ...
                  return (
                    <tr key={idx} className="hover:bg-[#252525] border-b border-[#2a2a2a] transition">
                      <td className="py-3.5 px-4 font-extrabold text-emerald-400">{wo.wo_number}</td>
                      <td className="py-3.5 px-4 font-extrabold text-white">{wo.item_name}</td>
                      <td className="py-3.5 px-4 text-slate-300 font-bold">{wo.customer_name || 'Hero MotoCorp'}</td>
                      <td className="py-3.5 px-4 text-right font-mono text-slate-300">{wo.planned_qty?.toLocaleString()} Pcs</td>
                      <td className="py-3.5 px-4 text-right font-black text-emerald-400">{wo.produced_qty?.toLocaleString() || 0} Pcs</td>
                      <td className="py-3.5 px-4 text-center">
                        <div className="flex items-center gap-2 justify-center">
                          <div className="w-28 bg-[#121212] h-2 rounded-full overflow-hidden border border-[#2a2a2a]">
                            <div className="bg-[#10b981] h-full rounded-full" style={{ width: `${progressPercent}%` }}></div>
                          </div>
                ...
                {woStatusReport.length === 0 && (
                  <tr>
                    <td colSpan="6" className="py-4 text-center text-slate-400 italic font-medium">No active work orders.</td>
                  </tr>
                )}
              </tbody>
            </table>
